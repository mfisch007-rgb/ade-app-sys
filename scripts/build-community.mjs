...
function excluded(full, name) {
  return /[\\/]node_modules[\\/]/.test(full) || /[\\/]\.git[\\/]/.test(full) || /[\\/]dist[\\/]/.test(full) || /[\\/]\.keys[\\/]/.test(full) || /(^|[\\/])\.env($|\.)/.test(name) || /\.(pem|key|p12|pfx|crt|cer|der|bundle|zip)$/i.test(name) || /credentials|secret/i.test(name) || /^ade_audit_persistence\./i.test(name) || /^\.ade_(storage|capability_store|session_revocations|feedback_queue)/i.test(name) || /(^|[\\/])\$log$/.test(full) || /^\$log$/.test(name) || /\.smoke-(err|out)\.log$/i.test(name) || /^\.opencode-final-checkpoint\.json$/i.test(name) || /^ADE_BATCH/i.test(name) || /^ADE-BATCH/i.test(name) || /^ADE-GIT-RELEASE-EVIDENCE/i.test(name) || /^ADE-RELEASE-CLOSURE-EVIDENCE/i.test(name) || /^ADE-CRITICAL-MISSING-FILES/i.test(name) || /^ADE_FINAL_TRUTH_GATE/i.test(name) || /^ADE_MASTER_FORENSIC/i.test(name) || /^ADE_MASTER_ENGINEERING_BATCH/i.test(name) || /^CLAUDE_BATCH/i.test(name) || /^RUN-ADE-ENGAGEMENT-RECONCILIATION/i.test(name) || /^live_root\.html$/i.test(name) || /^live-desktop\.png$/i.test(name) || /^public\/index\.html\.fixed$/i.test(full) || /backup/i.test(name) || /backup/i.test(full) || /^ade-(post-syntax-verification|safe-verification|verification-repair|server-forensic-backup|server-syntax-backup|reconciliation-backup)-/i.test(name) || /^\.ade-(batch-b-backup|reconciliation-backup)-/i.test(name) || /\.g(17aa4|18-httpsec|17z)-backup/i.test(name) || /^data2$/.test(name);
}
